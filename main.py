...
def importar_excel(file_path, obra_id):
    try:
        ...
        # Detecta a linha de cabeçalho (procura 'Codigo do Bem')
        header_idx = 0
        header = [str(v).strip() if v is not None else '' for v in linhas[0]]
        if 'Codigo do Bem' not in header:
            # tenta achar cabeçalho em outra linha (ex: primeira linha é lixo)
            for i, linha in enumerate(linhas[:5]):
                candidato = [str(v).strip() if v is not None else '' for v in linha]
                if 'Codigo do Bem' in candidato:
                    header = candidato
                    header_idx = i
                    break

        if 'Codigo do Bem' not in header or 'Descricao do Bem' not in header:
            return -1, 0, 0

        idx_codigo = header.index('Codigo do Bem')
        idx_descricao = header.index('Descricao do Bem')

        pares_limpos = []
        for linha in linhas[header_idx + 1:]:
            if linha is None:
                continue
            codigo_val = linha[idx_codigo] if idx_codigo < len(linha) else None
            descricao_val = linha[idx_descricao] if idx_descricao < len(linha) else None

            codigo_str = str(codigo_val).strip() if codigo_val is not None else ''
            descricao_str = str(descricao_val).strip() if descricao_val is not None else ''

            if not re.match(r'^[0-9]{3,}', codigo_str):
                continue
            if not descricao_str:
                continue
            if 'Estel Servicos' in descricao_str:
                continue

            pares_limpos.append((codigo_str, descricao_str))
        ...
        alterados = 0
        iguais = 0
        for codigo, descricao in pares_limpos:
...
def exportar_excel(obra_id):
    ...
    colunas = ['codigo', 'descricao', 'status', 'data_auditoria', 'observacoes', 'quantidade', 'local']
    ...
    ws.append(colunas)
    for item in dados:
        ws.append([item.get(col, '') for col in colunas])
